...
INNER JOIN shared_prod_datalake.GLOBALE_DBO.OrderStatuses OS ON O.OrderStatusId = OS.OrderStatusId
INNER JOIN shared_prod_datalake.GLOBALE_DBO.PaymentTransactions PT ON O.ActivePaymentTransactionId = PT.PaymentTransactionId
INNER JOIN shared_prod_datalake.GLOBALE_DBO.PaymentGateways PG ON PT.PaymentGatewayId = PG.PaymentGatewayId
INNER JOIN shared_prod_datalake.GLOBALE_DBO.ORDEREXTRADETAILS OED ON O.OrderId = OED.OrderId
INNER JOIN shared_prod_datalake.GLOBALE_SECURED.ORDERCUSTOMERDETAILS OCD ON O.OrderRecId = OCD.OrderRecId
INNER JOIN shared_prod_datalake.GLOBALE_DBO.Countries C ON O.ShipCountryId = C.CountryId
INNER JOIN shared_prod_datalake.GLOBALE_DBO.Merchants M ON O.MerchantId = M.MerchantId
INNER JOIN shared_prod_datalake.GLOBALE_DBO.Countries MC ON M.CountryId = MC.CountryId
INNER JOIN shared_prod_datalake.GLOBALE_DBO.States S ON O.ShipStateId = S.StateId
INNER JOIN shared_prod_datalake.GLOBALE_DBO.Stores ST ON O.StoreId = ST.StoreId
LEFT JOIN  shared_prod_datalake.GLOBALE_DBO.OrderCustomerDetails OCD2 ON OCD2.OrderRecId = O.OrderRecId
LEFT JOIN  shared_prod_datalake.GLOBALE_DBO.ORDERDELIVERYDETAILS ODD ON ODD.OrderId = O.OrderId AND ODD.ParcelId IS NULL
LEFT JOIN {{ ref('stg_lkp_reconciliationdate') }} RD ON RD.OrderId = O.OrderId
LEFT JOIN LATERAL (
    SELECT Case When Count(par.ParcelId) = 0 then 1 else Count(par.ParcelId) end as ParcelsCount
    FROM shared_prod_datalake.GLOBALE_DBO.Parcels par 
